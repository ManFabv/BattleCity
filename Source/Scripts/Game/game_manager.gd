...
@export var input_manager: PackedScene
...
func _ready() -> void:
	...
	var current_world : Node3D = world.instantiate() as Node3D
	# we add the nodes to the tree and then _ready is called in order
	add_child(current_world)
	# we instantiate the scenes for camera
	var current_player_camera : PlayerCamera = player_camera.instantiate() as PlayerCamera
	# we add the nodes to the tree and then _ready is called in order
	...
	var current_input_manager : InputManager = input_manager.instantiate() as InputManager
	# we add node to the tree and then _ready is called in order
	add_child(current_input_manager)
	# we instantiate the player
	var current_player : Player = player.instantiate() as Player
